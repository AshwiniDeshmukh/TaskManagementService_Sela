-- **********************************************************************************************************************************
-- RemovePatientAllergy.sql
-- 1. Determine whether the requested PatientAllergy exists in PatientAllergy table, finds only non-deleted records (i.e. the default is to find only records that are not deleted)
--    * If yes then remove the requested PatientAllergy key else raise an exception error not found.
-- 2. To remove PatientAllergy we update RecStatus to 0.
